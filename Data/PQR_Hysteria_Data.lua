...
-- Get Time Left on Icy Veins Function
PQ_GetIVLeft = nil
function PQ_GetIVLeft()
	local _,_,_,_,_,_,buffTime1 = UnitBuffID("player",PQ_IV)
	local _,_,_,_,_,_,buffTime2 = UnitBuffID("player",PQ_IVG)
	if buffTime1 then return buffTime1 - GetTime() end
	if buffTime2 then return buffTime2 - GetTime() end
	return 0
end

-- Get Cooldown on Icy Veins Function

PQ_GetIVCooldown = nil

function PQ_GetIVCooldown()
	local cd1 = spellCooldown((GetSpellInfo(PQ_IV)))
	local cd2 = spellCooldown((GetSpellInfo(PQ_IVG)))
	if cd1 then return cd1 end
	if cd2 then return cd2 end
	return 0
end

-- Get Time Left on Invocation Function
PQ_GetInvoLeft = nil
function PQ_GetInvoLeft()
	local _,_,_,_,_,_,buffTime1 = UnitBuffID("player",PQ_InvoBuff)
	if buffTime1 then return buffTime1 - GetTime() end
	return 0
end

PQ_GetBuffStacks = nil
function PQ_GetBuffStacks(spell)
	local _,_,_,count,_,_ = UnitBuffID("player",spell)
	if count then return count end
	return 0
end

PQ_GetDebuffStacks = nil
function PQ_GetDebuffStacks(spell)
	local _,_,_,count,_,_ = UnitDebuffID("player",spell)
	if count then return count end
	return 0
end

PQ_GetFoFLeft = nil
function PQ_GetFoFLeft()
	local _,_,_,_,_,buffTime1 = UnitBuffID("player",PQ_FoF)
	if buffTime1 then return buffTime1 - GetTime() end
	return 0
end
...
if select(2, UnitClass("player")) == "MAGE" then
	...
	if GetSpecialization("player") == 1 then
		...
		local Offensive = {
			...
			abilities = {
				...
				{ 	name	= "Auto Arcane Power",
					tooltip = "Toggle the automatic usage of Arcane Power.",
					enable	= true,
				},
	...
	PQ_Barrage		= 44425		-- Arcane Blast
	PQ_Missiles		= 5143		-- Arcane Missiles
	PQ_Blast		= 30451		-- Arcane Blast
...
function SpellSUCCEEDED(spellID,spellTARGET)
	...
	function fSSOnEvent(self,event,...)
		if event=="COMBAT_LOG_EVENT_UNFILTERED" then
			--local a1, subEvent, a2, sourceGUID, a3, a4, a5, destGUID, a6, a7, a8, lspellID, a10  = ...				
